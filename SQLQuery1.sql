...
--total drivers
select count(driverid) from trips;
...
select a.method from payment a inner join
(select top 1 faremethod fk from trips
order by fare desc) b
on a.id = b.fk;

-- which two locations had the most trips

select top 2 loc_from, loc_to, count(loc_from) from trips
group by loc_from,loc_to
order by count(loc_from) desc;

--top 5 earning drivers
select top 5 driverid, sum(fare) from trips
group by driverid
order by sum(fare) desc;


-- which duration had more trips
select a.duration from duration a inner join
(select top 1 duration, count(duration) dk from trips
group by duration
order by count(duration) desc) b
on a.id = b.dk;


-- which driver , customer pair had more orders
select * from
(select *, rank() over(order by dt desc) rak from 
(select driverid, custid, count(distinct tripid) dt from trips
...
select sum(searches_for_quotes)*100.00/sum(searches) from trips_details;
...
select loc_from, duration, count(distinct tripid) dt from trips
